...
- Pagination (`GET /bookings/`): prevents unbounded queries from returning unlimited data. `page_size` defaults to `DEFAULT_PAGE_SIZE` (250) so Day/Week/Month omit it; Range/Requests override with a smaller value. Bounded queries get `X-Total-Count`; unbounded get `X-Has-More`, computed server-side via over-fetch-by-one — not a client-side `items.length === pageSize` guess (that heuristic false-positives when the remaining count exactly equals page_size). `X-Page-Size` reports whichever was actually used.
