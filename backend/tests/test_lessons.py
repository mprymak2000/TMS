# LessonUpdate has no enrollment_id/tutor_id — a lesson can't change who it was for or who taught it —
...
def test_create_lesson_required_fields(client, setup):
    _enrollment, _tutor, lesson = setup
    response = client.post("/lessons/", json=lesson)
    assert response.status_code == 201
    
    data = response.json()
    # passed in fields
    assert data["enrollment_id"] == lesson["enrollment_id"]
...
# hrs > 0, no fee_override, no tutor_pay_override
# nomral lesson, auto calculated fee and payout based on the enrollment rate and tutor pay rate
def test_create_lesson_no_overrides(client, setup):
    ...
    data = response.json()
    assert data["fee"] == lesson["hrs"] * enrollment["rate"]
...
#  calculated based on the ratio of tutor pay rate to the enrollment rate (protects tutor from unfairly low payout if the rate is much higher than tutor pay rate, and protects business from overpaying tutor if the rate is much lower than tutor pay rate)
def test_create_cancelled_lesson_fee_override(client, setup):
    ...
    data = response.json()
    assert data["fee"] == lesson["fee_override"]
    assert data["tutor_payout"] == lesson["fee_override"] * (tutor["pay_rate"] / enrollment["rate"])
    assert data["is_fee_overridden"]
    assert not data["is_tutor_payout_overridden"]
...
def test_create_lesson_missing_required_fields(client, setup):
    _enrollment, _tutor, lesson = setup
    lesson = {"enrollment_id": lesson["enrollment_id"], "date": lesson["date"]}
    response = client.post("/lessons/", json=lesson)
...
def test_create_lesson_enrollment_not_found(client, setup):
    _enrollment, _tutor, lesson = setup
    lesson = {**lesson, "enrollment_id" : 9999}
    response = client.post("/lessons/", json=lesson)
...
def test_get_lessons(client, setup):
    _enrollment, _tutor, lesson1 = setup
...
def test_get_lesson_by_id(client, setup):
    ...
    assert response.status_code == 200
    assert response.json()["enrollment_id"] == lesson["enrollment_id"]
...
def test_update_lesson(client, setup_update):
    enrollment, tutor, lesson = setup_update
    ...
    assert data["fee"] == lesson_updated["hrs"] * enrollment["rate"]
    assert not data["is_fee_overridden"]
...
def test_update_lesson_fee_override(client, setup_update):
    _enrollment, tutor, lesson = setup_update
    lesson_prev = {**lesson, "hrs": 1.5, "notes": "original lesson"}
...
def test_update_lesson_add_tutor_pay_override(client, setup_update):
    enrollment, _tutor, lesson = setup_update
...
def test_update_lesson_occurred_to_cancelled(client, setup_update):
    ...
    assert data["fee"] == lesson_updated["fee_override"]
    assert data["is_fee_overridden"]
    assert data["tutor_payout"] == lesson_updated["fee_override"] * (tutor["pay_rate"] / enrollment["rate"])
    assert not data["is_tutor_payout_overridden"]
...
def test_update_lesson_pay_status_flip(client, setup_update):
    _enrollment, _tutor, lesson = setup_update
    lesson_prev = {**lesson, "hrs": 1.5}
...
def test_bulk_create_lessons(client, setup):
    ...
    assert data[0]["fee"] == lessons[0]["hrs"] * enrollment["rate"]
    assert data[1]["fee"] == lessons[1]["hrs"] * enrollment["rate"]
    assert data[2]["fee"] == lessons[2]["fee_override"]
    assert data[2]["tutor_payout"] == lessons[2]["fee_override"] * (tutor["pay_rate"] / enrollment["rate"])

# atomicity: one bad enrollment_id → nothing created
def test_bulk_create_lessons_bad_enrollment_rolls_back(client, setup):
    _enrollment, _tutor, lesson = setup
    lessons = [
        {**lesson, "date": "2026-01-01"},
        {**lesson, "date": "2026-01-02", "enrollment_id": 9999},
    ]
    response = client.post("/lessons/bulk_create", json=lessons)
    assert response.status_code == 404
