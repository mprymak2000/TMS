...
- Opaque ids: `Booking`/`BookingSeries` have a `public_id` (UUID) alongside their internal integer PK; API responses (`id`, `series_id`, `rescheduled_to`) expose only `public_id` — the integer PK is never serialized. For a series-bound `Booking`, `public_id` is set explicitly at creation/materialization time (not computed at read time) to the composite form `"{series.public_id}:{unix_timestamp}"`, mirroring Google Calendar's recurring-instance-id scheme (`{baseEventId}_{timestamp}`) — write-once, not recomputed on every serialize. Standalone bookings just get a plain generated UUID.
  - **How Google's recurring instances actually behave — verified empirically against the live calendar, not from the docs.** Google's reference documents `recurringEventId` and `originalStartTime` but *not* the instance id format; the published id rules (base32hex, lowercase a–v and digits) don't even permit the underscore, so the format below is an undocumented implementation detail that a probe of real data confirmed. Don't re-derive this from the docs — they'll appear to contradict it.
    ...
    - **Editing one occurrence creates an *exception instance*, not a standalone.** It keeps `recurringEventId` pointing at the master, still comes back from `events().instances()`, and dies with the series.
    - **The id keeps the ORIGINAL slot after a move.** Observed: an instance with id `..._20250526T223000Z` whose `start` is `2025-05-27T18:30` — moved a day later, id unchanged. The real time lives in `start`; `originalStartTime` is the stable grid handle, which is what the docs mean by "uniquely identifies the instance ... even if the instance was moved to a different time."
    - **Consequence for us, and it's the same design:** our composite collapses Google's two identity fields into one string, so leaving `public_id` stale after an admin moves an occurrence is exactly what Google does — the trailing timestamp means *original grid slot*, not *current time*. It also keeps already-emailed manage links working, since `resolve_ref` matches on `public_id` directly before ever parsing the timestamp.
    - Re-verify with a read-only `events().instances()` probe against a series that has a moved occurrence; `gcal.py:10` gives the service.
