...
- Pagination (`GET /bookings/`): prevents unbounded queries from returning unlimited data. `page_size` defaults to `DEFAULT_PAGE_SIZE` (250) so Day/Week/Month omit it; Range/Requests override with a smaller value. Response body (`BookingListResponse`) carries `total` (bounded queries only), `has_more` (unbounded only, computed server-side via over-fetch-by-one — not a client-side `items.length === pageSize` guess, which false-positives when the remaining count exactly equals page_size), and `page_size` (whichever was actually used). No pagination metadata in headers.
