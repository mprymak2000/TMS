...
- **Schedule** — referenced by `BookingLinkAvailability` (DB FK is `CASCADE`, but `delete_schedule` 409s first while any link exists — the cascade only ever fires for an already-unlinked schedule, so this behaves as a RESTRICT from the outside); the *default* schedule additionally can't be deleted at all until another schedule is made default first.
...
**BookingLink data model** — **partially shipped.** Pass 1 landed the rename off `EventType`, the `status` lifecycle, `slug`, and reassignment. Pass 2 (in progress) adds the `booking_types` table and the kind facet. Pass 3 (policy — the two-level `Settings` default with per-link override, resolved once at creation) is **not built**; the four policy columns exist on the link but `NULL` still means `auto` rather than "inherit the business default." The intake-form tables (`form_fields`, `booking_link_fields`, `event_field_responses`) and the per-booking policy freeze are also unbuilt — contact info still lives as fixed columns on `Booking`/`BookingSeries`, and `cancel_action`/`reschedule_action` still resolve live off the link.
...
**BookingLink recurrence modes** — three mutually exclusive modes on `recurring=True` links:
