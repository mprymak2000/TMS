...
**BookingLink data model** — **partially shipped.** Pass 1 landed the rename off `EventType`, the `status` lifecycle, `slug`, and reassignment. Pass 2 landed the `booking_types` table and the kind facet. Pass 3 (policy — the two-level `Settings` default with per-link override, resolved once at creation) is **not built**; the four policy columns exist on the link but `NULL` still means `auto` rather than "inherit the business default." The intake-form tables (`form_fields`, `booking_link_fields`, `event_field_responses`) and the per-booking policy freeze are also unbuilt — contact info still lives as fixed columns on `Booking`/`BookingSeries`, and `cancel_action`/`reschedule_action` still resolve live off the link.
