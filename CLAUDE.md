...
- **EventType** — **soft delete only, never hard-deleted** (planned — see "EventType data model" below; not yet implemented, `delete_event_type` currently has no guard at all and raises a raw unhandled `IntegrityError`/500 if any bookings exist). `EventType` carries the *identity* of a booking ("what is this"), and `Booking.event_type_id` is the live FK the entire filtering/facet system groups on — so the row must always resolve, forever. Retiring one sets `status='archived'`: it disappears from the booking page and admin create-flows, but the row persists and historical bookings keep resolving correctly. Nothing load-bearing is lost by archiving instead of deleting, because everything else a booking needs after creation is either copied onto it (policy, contact info, field answers) or independently anchored (`calendar_rules_id`). `EventTypeAvailability` rows still `CASCADE` on the FK, but that's moot once hard delete is off the table.
...
- **Known limitation, not being addressed now**: one global `Settings.business_timezone` assumes a single-location business. A business spanning multiple physical locations in different timezones (a franchise model) isn't representable — that would need timezone scoped per-tutor (or a future `Location` entity) rather than one app-wide value, and every booking would need its zone denormalized at creation from whichever tutor produced it, same freeze-at-creation pattern as the planned per-booking policy denormalization ("EventType data model" below). Deliberately deferred — no multi-location feature is currently planned. See `tms-roadmap` skill.

**Invariant: a past booking is never inert.** An admin can reschedule an already-happened booking into the future — `reschedule_booking` (`routers/bookings.py`) checks only `status != "confirmed"`, never whether `start` is in the past, and that's deliberate (the past-time notice floor is a booker-facing rule that intentionally doesn't apply to admin). The moment that happens the slot picker runs again, so rules get read again.

Consequences, all load-bearing — **"everything under this row is in the past" never means "nothing depends on it anymore":**
- `calendar_rules_id` on a booking must resolve **forever**, not just while the booking is upcoming. This is why the orphaned-`CalendarRules` cleanup job was rejected (see the `tms-roadmap` skill's Decisions section).
- `EventTypeAvailability` (which schedule applies to which tutor for this type) must survive too, since the picker needs it. That's what makes soft-delete-never-hard-delete on `EventType` load-bearing rather than merely tidy.
- Any future optimization scoped to "only active/future rows" is wrong by construction. Scope by *reference existence*, never by time.
- Policy is the one exception, and not because of timing: it's frozen onto the booking at creation and **copies forward on reschedule** (the original booking's terms, never the type's current ones), so it resolves correctly regardless of when the reschedule happens or what state its `EventType` is in.

**EventType data model** — **planned, not yet implemented.** Today every field below lives flat on `EventType`: no `status`, no fork mechanism, no separate rules table. Current shipped behavior: duration is either fixed (`duration_minutes`) or a custom range (`min`/`max_duration_minutes`), name is unique, business logic validated at the router layer.

Core principle: fields are split by **when they get read**, not by what they're about. Four buckets, each with its own resolution rule:

| Bucket | Question it answers | Resolution |
|---|---|---|
| **Identity** (WHAT) | What is this called | Live; forks on deliberate identity change |
| **Calendar rules** (HOW) | How should bookings be generated *right now* | Live via its own table; frozen by access-loss when the type is archived |
| **Policy** (WHEN) | What was promised to *this specific client* | Business default in `Settings`, optional per-type override, resolved once and frozen onto the booking |
| **Client info** (WHO) | Who is this booking for | Mixed: fixed contact columns + reusable label/answer fields |

**The filter key for the entire system is `bookings.event_type_id`** — a live FK. Nothing else is filtered on. `event_types` is the reusable blueprint; a `Booking` is one instance of it being used.

Schema sketch:

```
event_types                           [IDENTITY — live, forks on relaunch]
  id, name, description
  status                              active | archived
  predecessor_id -> event_types.id    nullable, optional lineage
  calendar_rules_id -> event_type_calendar_rules.id

event_type_calendar_rules             [CALENDAR RULES — live, group-frozen at type death]
  id, duration_minutes
  buffer_before, buffer_after, slot_increment
  max_per_day, max_per_week, min_lead_time, booking_horizon
  -- editable only while >=1 active type points at it

event_type_fields                     [CLIENT FIELDS — live definition]
  field_id -> form_fields.id          shared reusable question library
  event_type_id -> event_types.id
  sort_order, is_required, visible_to

bookings          -- and booking_series: EVERY field below exists on BOTH
  event_type_id -> event_types.id                     <- THE filter key
  calendar_rules_id -> event_type_calendar_rules.id   copied at creation
  ...
  cancel_allowed, cancel_cutoff_min                   policy, denormalized at creation
...
event_field_responses                 [CLIENT FIELDS — frozen answers]
  booking_id, field_id (nullable)
  label_snapshot                      question text as it was asked
  value
```

**Identity (`name`, `description`)** — live while the type is unchanged. Editing is a plain `UPDATE`, and every booking past and future reflects it immediately since they all read through the same live FK. An identity *change* (this isn't the same service anymore — "15-min Consult" drifting into "2-hr Group Meetup") is a **separate, deliberate admin action**, never inferred from a diff:
- **"Update all (past and future)"** → mutate the row in place. Same `id`, new name, done.
- **"Update future only"** → **archive-and-relaunch (fork)**. The old row gets `status='archived'`; a new row is inserted with a new id and *the same* `calendar_rules_id` carried over unchanged. Old bookings keep pointing at the archived id and still resolve; new bookings get the new id.

There is no algorithmic way to detect when an edit crosses from cosmetic to identity-changing, so these must be two distinct UI affordances — never inferred from the size of the change.

**`EventTypeAvailability` on fork — copy, never move.** The fork gets its own copy of every `(tutor_id, schedule_id)` row; the archived type keeps its original set. Moving the rows onto the fork would leave the archived type unable to resolve which schedule applies, which breaks rescheduling a past booking under it (see the invariant above). Availability is deliberately *not* shared across forks the way `calendar_rules_id` is: if the new version of a service drops a tutor, old bookings with that tutor must still be reschedulable with them. Archived types' rows are frozen by access-loss — same mechanism as calendar rules, since an archived type can't be edited, nothing can mutate them.

Note the split: `schedule_id` is frozen, but the `Schedule` it points at stays live and mutable. "Which schedule applies" is fixed at fork time; "what hours that schedule currently contains" is read live — correct, since you shouldn't reschedule someone into hours the tutor no longer works. Consequence: `delete_schedule`'s existing 409 guard now also fires for archived types' rows and becomes effectively permanent — a schedule ever wired to any event type can't be hard-deleted. Acceptable, since schedules are editable in place, so deletion is rarely the operation you actually want.

**Calendar rules (duration, buffers, slot increment, caps, lead time, horizon)** — their own table, referenced by the type via FK:
- They describe the calendar *right now*, not a promise to any one client. Always read live by the slot generator, for both new bookings and reschedules.
- **Kept in a separate table specifically so an identity fork doesn't fork calendar behavior too.** A rename must never accidentally freeze old bookings' buffer/duration — old and new type rows point at the *same* `calendar_rules_id`. Deliberate consequence: edit a buffer after forking three times and **all** of those forks' bookings see the change, including the oldest.
- Frozen implicitly once every type pointing at it is archived — the only edit path was "through an active type," so no explicit freeze flag is needed.
- **Caps are the exception inside this bucket.** `max_per_day`/`max_per_week` are storable settings, but "are we at capacity" is never a stored value — always a live `COUNT(*)` against real `bookings` rows for that type/day. Under concurrency, gate the check-then-insert with a Postgres advisory lock keyed on `(type_id, date)`.
- **Reschedule needs the original booking's numbers, not the type's current live ones.** `bookings` carries its own `start_at`/`end_at` (duration frozen by construction), and the slot generator takes an explicit override signature for reschedule — `generate_slots(type_id, duration=X, buffer=Y, exclude_id=Z)` — versus plain `generate_slots(type_id)` for new bookings. (`exclude_id` is also what stops a booking blocking itself — see the top-priority backlog item in the `tms-roadmap` skill.)

**Policy (cancel/reschedule cutoffs, max reschedules)** — **two-level: business default with an optional per-event-type override, resolved once at creation and frozen onto both `Booking` and `BookingSeries`.** Not versioned, never live-referenced afterward.

The cancellation policy is a *business* policy, not a per-service one — "we require 24 hours notice" is what you tell clients on your site and in intake paperwork. Per-type exceptions exist (a free intro consult is flexible, a 10-session package is strict) but they're exceptions, not the primary axis. So the four fields live on the `Settings` singleton as the business default, and `EventType` keeps the same four columns where **`NULL` means "inherit the business default"** (a semantic change from today's `null = auto`).
...
Resolution happens exactly once, at creation, in a single helper (`resolve_policy(event_type, settings)`), because the freeze *is* the resolution point. After that `get_cancel_action`/`get_reschedule_action` read the booking's own frozen columns — no live two-level lookup in any hot path, no chance of fallback logic drifting between call sites.

**No `Policy` entity.** A standalone shared table was designed and dropped: the only thing it bought over columns was "edit once, applies to many," and the `Settings` default delivers exactly that without a new table, a new route, or copy-vs-share and fork semantics to reason about. See the `tms-roadmap` skill's Decisions section. The series row isn't decorative: `_ensure_occurrence` copies fields *off the series* when Procrastinate materializes each new occurrence of an infinite series, so the series row is what governs everything not yet generated (this is also why a policy backfill has to update series rows, not just bookings — see the `tms-roadmap` skill). This is a promise made to one client at one moment: reading it live would mean that tightening a cancel window next week retroactively strips rights from someone who already booked. Copy once, never touch again for that booking.

**Client info (WHO)** — two distinct things, not to be conflated:
- **Fixed contact fields** (`first_name`, `last_name`, `email`, `phone`) — plain columns on **both `Booking` and `BookingSeries`** (as they already are today), email/phone nullable. For logged-in users, copy their profile name/contact *at booking time* rather than referencing `user_id` live — consistent with the freeze-on-booking pattern elsewhere, since an old booking's client-facing info shouldn't drift when someone edits their profile later.
- **Custom intake questions** — a reusable label:answer system. `form_fields` is the shared question library (name, input type, options); `event_type_fields` is the live join saying which questions a type asks, in what order, required or not; `event_field_responses` stores one answer row per question per booking, with a `label_snapshot` of the question text *as it was asked*, so later renaming a question never rewrites the meaning of an old answer.

**Deletion rules under this model:**
- **Types** — soft delete only (`status='archived'`). Hard delete isn't needed once identity and calendar rules are structured this way, since nothing load-bearing lives only on the type row past the point of booking.
- **Calendar rules rows** — never hard-deleted by default (negligible size). See the `tms-roadmap` skill's "Decisions — Considered and Rejected" section for why the cleanup job was evaluated and rejected.
...
- **Form fields** — soft delete via `archived_at`. Removing one from a type's form just drops it from `event_type_fields`; existing `event_field_responses` keep their `label_snapshot` regardless.
- **Slugs / booking links** — unique forever, including archived types. Never recycle a freed slug into a different type; that silently breaks old bookmarked links.

**Precedents** — **Stripe Product/Price** is the direct model: Product (name/description) is mutable and edited in place, while Price is immutable, so changing pricing means creating a new Price and archiving the old one, with Subscriptions/Invoices referencing the specific Price id active at the time, forever. **Airline fare rules** are the same shape as calendar rules surviving type death — change/refund terms attach to a fare class, shared live while it sells, frozen for already-issued tickets once it retires. **Google Calendar Appointment Schedules** is a useful *negative* example: duration changes apply live to all bookings with no freeze, no versioning, and no policy concept at all — appropriate for one freelancer's calendar, wrong for a system that needs historical accuracy for past clients.

**Terminology used above** — *live FK*: booking reads the current value through a shared row, no freezing. *Denormalized (flat copy)*: value copied onto the booking at creation and frozen from that point, no history of prior values kept. *Versioned*: normalized, edits insert new rows instead of updating, old rows persist and stay inspectable (a true audit trail); archive-and-relaunch is versioning triggered manually and rarely, with an added dead/alive flag on the superseded row. *Override*: nullable field on the child that defaults to inheriting the live parent value and diverges only when explicitly set for that one instance — not used anywhere in this model today, but available if a future need arises (e.g. one client granted a custom cancel window).
